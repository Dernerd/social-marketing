...
class Wdsm_SocialMarketing {
	...
	private function _load_dependencies () {
		...
		foreach ($this->_services as $class => $service) {
			...
			if (!class_exists($realclass)) throw new Wdsm_Exception(sprintf(__("Nicht unterstützter Dienst: %s", 'wdsm'), $class));
...
/* ----- Handlers ----- */
	...
	public function add_custom_columns ($cols) {
		return array_merge($cols, array(
			'wdsm_type' => __('Typ', 'wdsm'),
			'wdsm_services' => __('Dienste', 'wdsm'),
		));
	}
	...
	public function fill_custom_columns ($col) {
		...
		if ('wdsm_type' == $col) echo ('download_url' == wdsm_getval($wdsm, 'type')) ? __('Download URL', 'wdsm') : __('Gutscheincode', 'wdsm');
	...
	public function register_post_type () {
		...
		register_post_type('social_marketing_ad', array(
			'labels' => array(
				...
				'singular_name' => __('Social Marketing Werbung', 'wdsm'),
				'add_new_item' => __('Neue Social Marketing-Anzeige hinzufügen', 'wdsm'),
				'edit_item' => __('Social Marketing-Anzeige bearbeiten', 'wdsm'),
			),
	...
	public function add_meta_boxes () {
		add_meta_box(
			'wdsm_services',
			__('Richte Social Marketing ein', 'wdsm'),
	...
	public function render_services_box () {
		...
		echo
			"<label for='wdsm_url' style='width:100%'>" . 
				__('URL zum Teilen', 'wdsm') .
				$help->add_tip(__('Dies ist die URL, die Deine Besucher mit ihren Freunden teilen.', 'wdsm')) .
			'</label> ' .
		...
		echo
			"<label for='wdsm_button_text' style='width:100%'>" . 
				...
				$help->add_tip(__('Füge einen Aufruf zum Handeln hinzu, damit Deine Besucher klicken!', 'wdsm')) .
			'</label> ' .
		...
		echo
			"<label for='wdsm_type' style='width:100%'>" . 
				__('Typ', 'wdsm') . 
				$help->add_tip(__('Wähle ob Du einen kostenlosen Download oder einen Gutscheincode anbietest.', 'wdsm')) .
			'</label> ' .
			"<select class='widefat' id='wdsm_type' name='wdsm[type]'>" .
				...
				"<option wdsm:for='wdsm_coupon_code' value='coupon_code' {$checked_cc}>" . __('Gutscheincode', 'wdsm') . '</option>' .
			"</select>" .
		'<br />';
		...
		echo
			"<label for='wdsm_share_text' style='width:100%'>" . 
				__('Dankeschön Text <amall>(wird nach der Freigabeaktion angezeigt)</small>', 'wdsm') . 
				$help->add_tip(__('Vielen Dank an Deine Benutzer für das Klicken auf Deinen Link.', 'wdsm')) .
			'</label> ' .
		...
		echo "<thead><tr>";
		echo '<th colspan="4">' . 
			__('Social Media Dienste', 'wdsm') . 
			$help->add_tip(__('Wähle einen oder mehrere Social Media-Dienste aus.', 'wdsm')) .
		'</th>';
		echo "</tr></thead>\n";
		echo '<tbody><tr>';
		$cnt = 1;
...
/* ----- Front-end dependencies ----- */
	...
	public function include_frontend_javascript () {
		...
		$wdsm_data = array(
			...
			"strings" => array(
				"close_button" => esc_js(__('Schließen', 'wdsm')), 
