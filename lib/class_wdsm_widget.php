...
class Wdsm_WidgetAdvert extends WP_Widget {
	...
	function __construct () {
		$widget_ops = array('classname' => __CLASS__, 'description' => __('Zeigt Deine ausgewählte Social Marketing-Anzeige an', 'wdgpo'));
		parent::__construct(__CLASS__, 'Social Marketing Werbung', $widget_ops);
	...
	function form ($instance) {
		...
		$html = '<p>';
		$html .= '<label for="' . $this->get_field_id('title') . '">' . __('Titel:', 'wdgpo') . '</label>';
		$html .= '<input type="text" name="' . $this->get_field_name('title') . '" id="' . $this->get_field_id('title') . '" class="widefat" value="' . $title . '"/>';
		$html .= '</p>';

		$html .= '<p>';
		$html .= '<label for="' . $this->get_field_id('ad_id') . '">' . __('Diese Anzeige anzeigen:', 'wdgpo') . '</label> ';
